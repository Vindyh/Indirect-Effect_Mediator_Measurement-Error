############################################################################################################
#
# Numerical Optimization Using Probit Link (Adjusted for measurement Error; using 2 assay limits)
# Updated: 05/19/2025
...
noptim_probit_delta_sca <- function(Y, M, C, delta, lod1, lod2, shift, initial1, initial2, sigma.u, source_A5345){
  ...
  fit <- optim(par = pars.init, fn = log.likelihood, control=list("fnscale"=-1), method = "L-BFGS-B", 
               lower = c(rep(-Inf, 2), 0.001, rep(-Inf, 3)), hessian = TRUE)
  ...
  denom_A13 <-  sqrt(1 + (beta.1^2 * sigma_M_sq))
